...
public class RTSGameController : MonoBehaviour
{
        ...
        if (mouse != null)
        {
            ...
            if (mouse.leftButton.isPressed)
            {
                ...
                if(Vector2.Distance(mouseClickStart, mousePosition) > mouseDragThreshold)
                {
                    ...
                    //Get drag bounds and assign size
                    Vector2 box = new Vector2(Mathf.Abs(
                        mouseClickStart.x - mousePosition.x), 
            ...
            {
                if (selectionBox.gameObject.activeInHierarchy) SelectInBox();
                else SelectUnderMouse();
    ...
    void SelectInBox()
    {
        ...
        foreach(RTSCharacterController character in characterControllers)
        {
            ...
            characterPosition /= uiCanvas.scaleFactor;
